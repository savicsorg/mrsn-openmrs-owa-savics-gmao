angular.module('RequestsController', ['ngMaterial', 'md.data.table']).controller('RequestsController', ['$scope', '$state', '$stateParams', '$rootScope', '$mdToast', 'openmrsRest', '$mdDialog', '$q', '$translate', function ($scope, $state, $stateParams, $rootScope, $mdToast, openmrsRest, $mdDialog, $q, $translate) {
    ...
    $scope.openEdit = function (data, canValidate) {
        console.log("Requests", canValidate)
    ...
    $scope.search = function (item) {
        if ($scope.searchAll == "" || ($scope.searchAll.length > 0 && item.equipment.name.toLowerCase().indexOf($scope.searchAll.toLowerCase()) > -1)) {
            if ($scope.requestValidated === true) {
    ...
    $scope.donwload = function () {
        let link = window.location.protocol + "//" + window.location.host + "/openmrs/ws/rest/v1/savicsgmao/maintenancerequests/export?requestValidated=" + $scope.requestValidated
